...
void Jelly::computeInternalForce() {
    ...
    for (const Spring &spring : springs) {
        ...
        Eigen::Vector3f posA = particles[startID].getPosition();
        Eigen::Vector3f posB = particles[endID].getPosition();
        Eigen::Vector3f velA = particles[startID].getVelocity();
        Eigen::Vector3f velB = particles[endID].getVelocity();

        float springCoef = spring.getSpringCoef();
        float damperCoef = spring.getDamperCoef();
        float restLength = spring.getSpringRestLength();

        // Compute forces
        Eigen::Vector3f springForce = computeSpringForce(posA, posB, springCoef, restLength);
        Eigen::Vector3f damperForce = computeDamperForce(posA, posB, velA, velB, damperCoef);

        // Apply forces
        particles[startID].addForce(springForce + damperForce);
        particles[endID].addForce(-springForce - damperForce);
    }
}
...
Eigen::Vector3f Jelly::computeSpringForce(const Eigen::Vector3f &positionA, const Eigen::Vector3f &positionB,
    ...
    Eigen::Vector3f spring_vector = positionA - positionB;
    float spring_length = spring_vector.norm();
    spring_force = -springCoef * (spring_length - restLength) * spring_vector / spring_length;
