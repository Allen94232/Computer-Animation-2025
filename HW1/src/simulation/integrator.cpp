#include "integrator.h"
#include <iostream>

#include <vector>
namespace simulation {
// Factory
std::unique_ptr<Integrator> IntegratorFactory::CreateIntegrator(IntegratorType type) {
    switch (type) {
        case simulation::IntegratorType::ExplicitEuler:
            return std::make_unique<ExplicitEulerIntegrator>();
        case simulation::IntegratorType::ImplicitEuler:
            return std::make_unique<ImplicitEulerIntegrator>();
        case simulation::IntegratorType::MidpointEuler:
            return std::make_unique<MidpointEulerIntegrator>();
        case simulation::IntegratorType::RungeKuttaFourth:
            return std::make_unique<RungeKuttaFourthIntegrator>();
        default:
            throw std::invalid_argument("TerrainFactory::CreateTerrain : invalid TerrainType");
            break;
    }
    return nullptr;
}

//
// ExplicitEulerIntegrator
//

IntegratorType ExplicitEulerIntegrator::getType() { return IntegratorType::ExplicitEuler; }

void ExplicitEulerIntegrator::integrate(MassSpringSystem& particleSystem) {
    // TODO#4-1: Integrate position and velocity
    //   1. Integrate position using current velocity.
    //   2. Integrate velocity using current acceleration.
    //   3. Clear force
    // Note:
    //   1. You should do this first because it is very simple. Then you can check whether your collision is correct or
    //   not.
    //   2. See functions in `particle.h` if you don't know how to access data.
    //   3. You can access data and function in particleSystem.elevatorTerrain
    //   4. Review "ODE_basics.pptx" from p.15 - p.16

    for (int jellyIdx = 0; jellyIdx < particleSystem.jellyCount; jellyIdx++) {
        Jelly& jelly = particleSystem.jellies[jellyIdx];
        for (int i = 0; i < jelly.getParticleNum(); i++) {
            Particle& particle = jelly.getParticle(i);
            // Integrate position
            particle.addPosition(particle.getVelocity() * particleSystem.deltaTime);
            // Integrate velocity
            particle.addVelocity(particle.getForce() / particle.getMass() * particleSystem.deltaTime);
            // Clear force
            particle.setForce(Eigen::Vector3f::Zero());
        }
    }

    particleSystem.elevatorTerrain->addPosition(particleSystem.elevatorTerrain->getVelocity() *
                                                particleSystem.deltaTime);
    particleSystem.elevatorTerrain->addVelocity(particleSystem.elevatorTerrain->getForce() /
                                                particleSystem.elevatorTerrain->getMass() * particleSystem.deltaTime);
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());

}

//
// ImplicitEulerIntegrator
//

IntegratorType ImplicitEulerIntegrator::getType() { return IntegratorType::ImplicitEuler; }

void ImplicitEulerIntegrator::integrate(MassSpringSystem& particleSystem) {
    // TODO#4-2: Integrate position and velocity
    //   1. Backup original particles' data.
    //   2. Integrate position and velocity using explicit euler to get Xn+1.
    //   3. Compute refined Xn+1 and Vn+1 using (1.) and (2.).
    // Note:
    //   1. Use `MassSpringSystem::computeJellyForce`and `MassSpringSystem::computeElevatorForce`
    //      with modified position and velocity to get Xn+1.
    //   2. See functions in `particle.h` if you don't know how to access data.
    //   3. You can access data and function in particleSystem.elevatorTerrain
    //   4. Remember to reset particleSystem.elevatorCounter back to the original state
    //   5. Review "ODE_implicit.pptx" from p.18 - p.19

    for (int jellyIdx = 0; jellyIdx < particleSystem.jellyCount; jellyIdx++) {
        Jelly& jelly = particleSystem.jellies[jellyIdx];
        for (int i = 0; i < jelly.getParticleNum(); i++) {
            Particle& particle = jelly.getParticle(i);
            // Backup original particles' data
            Eigen::Vector3f originalPosition = particle.getPosition();
            Eigen::Vector3f originalVelocity = particle.getVelocity();
            // Integrate position and velocity using explicit euler
            particle.addPosition(particle.getVelocity() * particleSystem.deltaTime);
            particle.addVelocity(particle.getForce() / particle.getMass() * particleSystem.deltaTime);
            // Compute refined Xn+1 and Vn+1
            Eigen::Vector3f refinedPosition = particle.getPosition();
            Eigen::Vector3f refinedVelocity = particle.getVelocity();
            particle.setPosition(originalPosition);
            particle.setVelocity(originalVelocity);
            particle.addPosition((refinedPosition - originalPosition) * 0.5);
            particle.addVelocity((refinedVelocity - originalVelocity) * 0.5);
            // Clear force
            particle.setForce(Eigen::Vector3f::Zero());
        }
    }

    // Elevator
    Eigen::Vector3f originalPosition = particleSystem.elevatorTerrain->getPosition();
    Eigen::Vector3f originalVelocity = particleSystem.elevatorTerrain->getVelocity();
    particleSystem.elevatorTerrain->addPosition(particleSystem.elevatorTerrain->getVelocity() *
                                                particleSystem.deltaTime);
    particleSystem.elevatorTerrain->addVelocity(particleSystem.elevatorTerrain->getForce() /
                                                particleSystem.elevatorTerrain->getMass() * particleSystem.deltaTime);
    Eigen::Vector3f refinedPosition = particleSystem.elevatorTerrain->getPosition();
    Eigen::Vector3f refinedVelocity = particleSystem.elevatorTerrain->getVelocity();
    particleSystem.elevatorTerrain->setPosition(originalPosition);
    particleSystem.elevatorTerrain->setVelocity(originalVelocity);
    particleSystem.elevatorTerrain->addPosition((refinedPosition - originalPosition) * 0.5);
    particleSystem.elevatorTerrain->addVelocity((refinedVelocity - originalVelocity) * 0.5);
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
}

//
// MidpointEulerIntegrator
//

IntegratorType MidpointEulerIntegrator::getType() { return IntegratorType::MidpointEuler; }

void MidpointEulerIntegrator::integrate(MassSpringSystem& particleSystem) {
    // TODO#4-3: Integrate position and velocity
    //   1. Backup original particles' data.
    //   2. Integrate position and velocity using explicit euler to get Xn+1.
    //   3. Compute refined Xn+1 using (1.) and (2.).
    // Note:
    //   1. Use `MassSpringSystem::computeJellyForce`and `MassSpringSystem::computeElevatorForce`
    //      with modified position and velocity to get Xn+1.
    //   2. See functions in `particle.h` if you don't know how to access data.
    //   3. You can access data and function in particleSystem.elevatorTerrain
    //   4. Remember to reset particleSystem.elevatorCounter back to the original state
    //   5. Review "ODE_basics.pptx" from p.18 - p.19

    for (int jellyIdx = 0; jellyIdx < particleSystem.jellyCount; jellyIdx++) {
        Jelly& jelly = particleSystem.jellies[jellyIdx];
        for (int i = 0; i < jelly.getParticleNum(); i++) {
            Particle& particle = jelly.getParticle(i);
            // Backup original particles' data
            Eigen::Vector3f originalPosition = particle.getPosition();
            Eigen::Vector3f originalVelocity = particle.getVelocity();
            // Integrate position and velocity using explicit euler
            particle.addPosition(particle.getVelocity() * particleSystem.deltaTime);
            particle.addVelocity(particle.getForce() / particle.getMass() * particleSystem.deltaTime);
            // Compute refined Xn+1
            Eigen::Vector3f refinedPosition = particle.getPosition();
            particle.setPosition(originalPosition);
            particle.addPosition((refinedPosition - originalPosition) * 0.5);
            // Clear force
            particle.setForce(Eigen::Vector3f::Zero());
        }
    }

    // Elevator
    Eigen::Vector3f originalPosition = particleSystem.elevatorTerrain->getPosition();
    Eigen::Vector3f originalVelocity = particleSystem.elevatorTerrain->getVelocity();
    particleSystem.elevatorTerrain->addPosition(particleSystem.elevatorTerrain->getVelocity() *
                                                particleSystem.deltaTime);
    particleSystem.elevatorTerrain->addVelocity(particleSystem.elevatorTerrain->getForce() /
                                                particleSystem.elevatorTerrain->getMass() * particleSystem.deltaTime);
    Eigen::Vector3f refinedPosition = particleSystem.elevatorTerrain->getPosition();
    particleSystem.elevatorTerrain->setPosition(originalPosition);
    particleSystem.elevatorTerrain->addPosition((refinedPosition - originalPosition) * 0.5);
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
}

//
// RungeKuttaFourthIntegrator
//

IntegratorType RungeKuttaFourthIntegrator::getType() { return IntegratorType::RungeKuttaFourth; }

void RungeKuttaFourthIntegrator::integrate(MassSpringSystem& particleSystem) {
    struct StateStep {
        Eigen::Vector3f deltaVel;
        Eigen::Vector3f deltaPos;
    };
    // TODO#4-4: Integrate velocity and acceleration
    //   1. Backup original particles' data.
    //   2. Compute k1, k2, k3, k4
    //   3. Compute refined Xn+1 using (1.) and (2.).
    // Note:
    //   1. Use `MassSpringSystem::computeJellyForce`and `MassSpringSystem::computeElevatorForce`
    //      with modified position and velocity to get Xn+1.
    //   2. See functions in `particle.h` if you don't know how to access data.
    //   3. You can access data and function in particleSystem.elevatorTerrain
    //   4. Remember to reset particleSystem.elevatorCounter back to the original state
    //   5. StateStep struct is just a hint, you can use whatever you want.
    //   6. Review "ODE_basics.pptx" from p.21

     StateStep step;
    int particle_ct = particleSystem.getJellyPointer(0)->getParticleNum();  // jelly index = 0
    std::vector<Particle> previous_p;
    std::vector<Particle> new_p;

    for (int i = 0; i < particle_ct; i++) {
        previous_p.push_back(particleSystem.getJellyPointer(0)->getParticle(i));
        new_p.push_back(particleSystem.getJellyPointer(0)->getParticle(i));
    }

    // k1
    for (int i = 0; i < particle_ct; i++) {
        Particle* single_p = &particleSystem.getJellyPointer(0)->getParticle(i);

        // a' = g + f/m
        Eigen::Vector3f accel = (single_p->getForce() / single_p->getMass());
        Eigen::Vector3f pos = single_p->getPosition();
        Eigen::Vector3f velocity = single_p->getVelocity();
        float delta_t = particleSystem.deltaTime;

        // Integrate position first , x' = x + vt
        step.deltaPos = velocity * delta_t;
        // Integrate velocity , v' = v + at
        step.deltaVel = accel * delta_t;

        // set new position & velocity
        single_p->setAcceleration(accel);
        // single_p->setPosition(pos);
        single_p->setVelocity(previous_p[i].getVelocity() + step.deltaVel);  //
        // since acceleration is added
        single_p->setForce(Eigen::Vector3f::Zero());
        new_p[i].addPosition(step.deltaPos / 6.0);
        new_p[i].addVelocity(step.deltaVel / 6.0);
    }
    // k2
    particleSystem.computeJellyForce(*particleSystem.getJellyPointer(0));
    for (int i = 0; i < particle_ct; i++) {
        Particle* single_p = &particleSystem.getJellyPointer(0)->getParticle(i);

        // a' = g + f/m
        Eigen::Vector3f accel = (single_p->getForce() / single_p->getMass());
        Eigen::Vector3f pos = single_p->getPosition();
        Eigen::Vector3f velocity = single_p->getVelocity();
        float delta_t = particleSystem.deltaTime;

        // Integrate position first , x' = x + vt
        step.deltaPos = velocity * delta_t;
        // Integrate velocity , v' = v + at
        step.deltaVel = accel * delta_t;

        // set new position & velocity
        single_p->setAcceleration(accel);
        // single_p->setPosition(pos);
        single_p->setVelocity(previous_p[i].getVelocity() + 0.5 * step.deltaVel);
        // since acceleration is added
        single_p->setForce(Eigen::Vector3f::Zero());
        new_p[i].addPosition(step.deltaPos / 3.0);
        new_p[i].addVelocity(step.deltaVel / 3.0);
    }
    // k3
    particleSystem.computeJellyForce(*particleSystem.getJellyPointer(0));
    for (int i = 0; i < particle_ct; i++) {
        Particle* single_p = &particleSystem.getJellyPointer(0)->getParticle(i);

        // a' = g + f/m
        Eigen::Vector3f accel = (single_p->getForce() / single_p->getMass());
        Eigen::Vector3f pos = single_p->getPosition();
        Eigen::Vector3f velocity = single_p->getVelocity();
        float delta_t = particleSystem.deltaTime;

        // Integrate position first , x' = x + vt
        step.deltaPos = velocity * delta_t;
        // Integrate velocity , v' = v + at
        step.deltaVel = accel * delta_t;

        // set new position & velocity
        single_p->setAcceleration(accel);
        // single_p->setPosition(pos);
        single_p->setVelocity(previous_p[i].getVelocity() + 0.5 * step.deltaVel);  //
        // since acceleration is added
        single_p->setForce(Eigen::Vector3f::Zero());
        new_p[i].addPosition(step.deltaPos / 3.0);
        new_p[i].addVelocity(step.deltaVel / 3.0);
    }
    // k4
    particleSystem.computeJellyForce(*particleSystem.getJellyPointer(0));
    for (int i = 0; i < particle_ct; i++) {
        Particle* single_p = &particleSystem.getJellyPointer(0)->getParticle(i);

        // a' = g + f/m
        Eigen::Vector3f accel = (single_p->getForce() / single_p->getMass());
        Eigen::Vector3f pos = single_p->getPosition();
        Eigen::Vector3f velocity = single_p->getVelocity();
        float delta_t = particleSystem.deltaTime;

        // Integrate position first , x' = x + vt
        step.deltaPos = velocity * delta_t;
        // Integrate velocity , v' = v + at
        step.deltaVel = accel * delta_t;

        // set new position & velocity
        single_p->setAcceleration(accel);
        // single_p->setPosition(pos);
        single_p->setVelocity(previous_p[i].getVelocity() + step.deltaVel);  //
        // since acceleration is added
        single_p->setForce(Eigen::Vector3f::Zero());
        new_p[i].addPosition(step.deltaPos / 6.0);
        new_p[i].addVelocity(step.deltaVel / 6.0);
    }

    // update
    for (int i = 0; i < particle_ct; i++) {
        Particle* single_p = &particleSystem.getJellyPointer(0)->getParticle(i);
        Eigen::Vector3f pos = new_p[i].getPosition();
        Eigen::Vector3f velocity = new_p[i].getVelocity();
        single_p->setPosition(pos);
        single_p->setVelocity(velocity);
        single_p->setForce(Eigen::Vector3f::Zero());
    }

    // Elevator
    StateStep step_elevator;
    // k1
    // a' = g + f/m
    Eigen::Vector3f accel = (particleSystem.elevatorTerrain->getForce() / particleSystem.elevatorTerrain->getMass());
    Eigen::Vector3f pos = particleSystem.elevatorTerrain->getPosition();
    Eigen::Vector3f velocity = particleSystem.elevatorTerrain->getVelocity();
    float delta_t = particleSystem.deltaTime;

    // Integrate position first , x' = x + vt
    step_elevator.deltaPos = velocity * delta_t;
    // Integrate velocity , v' = v + at
    step_elevator.deltaVel = accel * delta_t;

    // set new position & velocity
    particleSystem.elevatorTerrain->setAcceleration(accel);
    particleSystem.elevatorTerrain->setVelocity(velocity + step_elevator.deltaVel);  //
    // since acceleration is added
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
    particleSystem.elevatorTerrain->addPosition(step_elevator.deltaPos / 6.0);
    particleSystem.elevatorTerrain->addVelocity(step_elevator.deltaVel / 6.0);

    // k2
    particleSystem.computeElevatorForce();
    // a' = g + f/m
    accel = (particleSystem.elevatorTerrain->getForce() / particleSystem.elevatorTerrain->getMass());
    pos = particleSystem.elevatorTerrain->getPosition();
    velocity = particleSystem.elevatorTerrain->getVelocity();
    delta_t = particleSystem.deltaTime;

    // Integrate position first , x' = x + vt
    step_elevator.deltaPos = velocity * delta_t;
    // Integrate velocity , v' = v + at
    step_elevator.deltaVel = accel * delta_t;

    // set new position & velocity
    particleSystem.elevatorTerrain->setAcceleration(accel);
    particleSystem.elevatorTerrain->setVelocity(velocity + 0.5 * step_elevator.deltaVel);
    // since acceleration is added
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
    particleSystem.elevatorTerrain->addPosition(step_elevator.deltaPos / 3.0);
    particleSystem.elevatorTerrain->addVelocity(step_elevator.deltaVel / 3.0);

    // k3
    particleSystem.computeElevatorForce();
    // a' = g + f/m
    accel = (particleSystem.elevatorTerrain->getForce() / particleSystem.elevatorTerrain->getMass());
    pos = particleSystem.elevatorTerrain->getPosition();
    velocity = particleSystem.elevatorTerrain->getVelocity();
    delta_t = particleSystem.deltaTime;

    // Integrate position first , x' = x + vt
    step_elevator.deltaPos = velocity * delta_t;
    // Integrate velocity , v' = v + at
    step_elevator.deltaVel = accel * delta_t;

    // set new position & velocity
    particleSystem.elevatorTerrain->setAcceleration(accel);
    particleSystem.elevatorTerrain->setVelocity(velocity + 0.5 * step_elevator.deltaVel);  //
    // since acceleration is added
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
    particleSystem.elevatorTerrain->addPosition(step_elevator.deltaPos / 3.0);
    particleSystem.elevatorTerrain->addVelocity(step_elevator.deltaVel / 3.0);

    // k4
    particleSystem.computeElevatorForce();
    // a' = g + f/m
    accel = (particleSystem.elevatorTerrain->getForce() / particleSystem.elevatorTerrain->getMass());
    pos = particleSystem.elevatorTerrain->getPosition();
    velocity = particleSystem.elevatorTerrain->getVelocity();
    delta_t = particleSystem.deltaTime;

    // Integrate position first , x' = x + vt
    step_elevator.deltaPos = velocity * delta_t;
    // Integrate velocity , v' = v + at
    step_elevator.deltaVel = accel * delta_t;

    // set new position & velocity
    particleSystem.elevatorTerrain->setAcceleration(accel);
    particleSystem.elevatorTerrain->setVelocity(velocity + step_elevator.deltaVel);  //
    // since acceleration is added
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
    particleSystem.elevatorTerrain->addPosition(step_elevator.deltaPos / 6.0);
    particleSystem.elevatorTerrain->addVelocity(step_elevator.deltaVel / 6.0);

    // update
    Eigen::Vector3f elevatorPos = particleSystem.elevatorTerrain->getPosition();
    Eigen::Vector3f elevatorVel = particleSystem.elevatorTerrain->getVelocity();
    particleSystem.elevatorTerrain->setPosition(elevatorPos);
    particleSystem.elevatorTerrain->setVelocity(elevatorVel);
    particleSystem.elevatorTerrain->setForce(Eigen::Vector3f::Zero());
}
}  // namespace simulation
